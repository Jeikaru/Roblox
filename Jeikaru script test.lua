...
print("[KarmaPanda] Key Check: " .. os.clock() - benchmark_time)
...
if _G.Alternative then
    rayfield_data = game:HttpGet(base_url .. 'arrayfield')
else
    rayfield_data = game:HttpGet(
                        'https://raw.githubusercontent.com/KarmaPanda/Rayfield/main/ArrayField_min.lua')
    if rayfield_data == nil then
        rayfield_data = game:HttpGet(base_url .. 'arrayfield')
    end
end

local Rayfield = loadstring(rayfield_data)()
...
local DefaultSettings = {
    ...
    auto_battle = false,
    battle_gems = 2700,
    automatic_upgrade_wave = 1,
    automatic_upgrade_money = 100,
    automatic_upgrade_speed = 0.4,
    automatic_upgrade_delay = 1,
...
local function get_game_status()
    ...
    local status = end_gui:WaitForChild("BG"):WaitForChild("Status")
                       :WaitForChild("Status")
...
local function ElapsedTime()
    local wave = game:GetService("ReplicatedStorage")
                     :FindFirstChild("WaveValue")
...
local function CalculateTimeOffset()
    task.spawn(function()
        repeat task.wait() until get_game_speed() ~= nil and
            game:GetService("ReplicatedStorage"):WaitForChild("WaveValue").Value >
            0
...
local function UpgradeUnit(unit, upgrade_level)
    local status, err = pcall(function()
        ...
        if Settings.macro_money_tracking then
            ...
            local total_upgrade_levels = upgrade_level -
                                             unit_upgrade_level.Value
        ...
        for i = unit_upgrade_level.Value + 1, upgrade_level do
            Upgrade_Counter = Upgrade_Counter + 1
            AddToQueue(game:GetService("ReplicatedStorage").Remotes.Server,
                       {[1] = "Upgrade", [2] = unit})
        end
...
local function SkipWave(wave)
    task.spawn(function()
        ...
        while game:GetService("ReplicatedStorage"):WaitForChild("WaveValue").Value ==
            wave and GUI.HUD.NextWaveVote.Visible do
            AddToQueue(game:GetService("ReplicatedStorage").Remotes.Input,
                       {[1] = "VoteWaveConfirm"})
            task.wait(1)
        end
    ...
    if HUD ~= nil then
        ...
        if yes_button ~= nil then
            return yes_button.MouseButton1Click:Connect(function()
                local WaveValue = game:GetService("ReplicatedStorage")
                                      :FindFirstChild("WaveValue")
                local wave = 0
                ...
                if Settings.macro_record and Settings.macro_skipwave then
                    InsertToMacro({
                        ...
                        ["Parameter"] = {["Wave"] = WaveValue.Value}
                    })
                end
            end)
        end
...
local function HookAutoSkipWave()
    local Toggle = GUI:WaitForChild("HUD"):WaitForChild("Setting"):WaitForChild(
                       ...
                       :WaitForChild("AutoSkip"):WaitForChild("Options")
                       :WaitForChild("Toggle")
...
-- TODO: Add Macro Recording for 2/3x speed changes
...
function AutoBattle() -- W.I.P the setting for this is on line 2400 and the toggle is on line 2728
    local function buyAutoBattle()
        return game:GetService("ReplicatedStorage").Remotes.Input:FireServer("BuyAutoBattle")
    end
    
    if getgems() < Settings.battle_gems then
        Settings.auto_battle = false
        Rayfield:Notify({
            Title = "You have no gems to run the script!",
            Content = "Get More Gems!",
            Duration = 6.5,
            Image = 4483362458,
        ...
        if pcall(buyAutoBattle) then
            task.wait()
        else
            warn("Failed to run the remote")
        end
    end
            ...
            table.insert(webhook_args, {
                ...
                ["value"] = ":ocean: " ..
                    game:GetService("ReplicatedStorage")
                        :FindFirstChild("WaveValue").Value,
                ["inline"] = true
            })
...
function AutoJoinGame()
    ...
    if get_world() == 1 then
        ...
        local function TeleportToWorld2()
            UseTeleporter(game:GetService("Workspace").Queue["W2 PERM"].World2
                              .Script115)
        end
    ...
    elseif get_world() == 2 then
        ...
        local function TeleportToWorld1()
            UseTeleporter(game:GetService("Workspace").Script115)
        end
        local teleporter = nil
        if Settings.auto_join_mode == "Story" then
...
function TogglePlayerName()
    game:GetService("Workspace").Camera:WaitForChild(Player.Name).Head:WaitForChild(
        "NameLevelBBGUI"):WaitForChild("NameFrame"):WaitForChild("TextLabel")
        .Visible = not Settings.hide_player_name
end
...
if get_world() ~= -1 and get_world() ~= -2 then
    ...
    if not is_lobby() then
        ...
        if JSON.automatic_upgrade then task.spawn(AutoUpgrade) end
    ...
    if Settings.hide_player_name then task.spawn(TogglePlayerName) end
end
...
local function CreateHideButtonGUI()
    ...
    Frame.Position = UDim2.new(0.033, 0, 0.305, 0)
    ...
    ImageLabel.Image = "http://www.roblox.com/asset/?id=16739811284"
    ...
    Frame.InputChanged:Connect(function(input)
        if isDragging and input.UserInputType == Enum.UserInputType.Touch then
            ...
            Frame.Position = UDim2.new(frameStartPos.X.Scale,
...
local function CreateMiniGUI()
    ...
    playbackLabel.Size = UDim2.new(1, 0, 0.55, 0)
    playbackLabel.Position = UDim2.new(0, 0, 0.45, 0)
    playbackLabel.BackgroundTransparency = 0.5
    ...
    local function Update()
        ...
        if currentTime - lastUpdateTime >= updateInterval then
            ...
            local pingString = game:GetService("Stats"):WaitForChild("Network")
                                   :WaitForChild("ServerStatsItem")
                                   :WaitForChild("Data Ping"):GetValueString()
            local ping = tonumber(string.split(pingString, " ")[1])
                ...
                if MacroCurrentStep ~= nil then
                    ...
                    if MacroCurrentStep["Parameter"] ~= nil then
                        ...
                        for k, v in pairs(MacroCurrentStep["Parameter"]) do
                            Parameters =
                                Parameters .. tostring(k) .. ": " .. tostring(v) ..
                                    "; "
                        end
                    end
...
function InitializeUI()
    ...
    local function MainSettings()
        ...
        local AutomaticUpgradeToggle = Main:CreateToggle({
            Name = "Auto Upgrade",
            CurrentValue = Settings.automatic_upgrade,
            Callback = function(value)
                Settings.automatic_upgrade = value
                Save()
    ...
    local function MiscellaneousSettings()
        ...
        if get_world() ~= -1 and get_world() ~= -2 then
            Miscellaneous:CreateToggle({
                ...
                Info = "Hides player name for those who want to record or screenshot",
                CurrentValue = Settings.hide_player_name,
                Callback = function(value)
                    Settings.hide_player_name = value
                    Save()
                    TogglePlayerName()
                end,
                SectionParent = GameSettingsSection
        ...
        Miscellaneous:CreateButton({
            ...
            Callback = function()
                ...
                Rayfield:Notify({
                    Title = "Reset",
                    Content = "Settings restored to default! Please restart game to see setting changes.",
                    Duration = 6.5,
                    Image = 4483362458
                })
            end,
...
if Settings.auto_execute then library.AutoExecute() end
